...
func Getstringtoint(stringtoint string) int64 {
	num, err := strconv.ParseInt(stringtoint, 10, 64)
	if err == nil {
		return num
	}
	return -1
...
func (c *MainController) Get() {
	...
	var page int64 = 0
	page_size := beego.AppConfig.String("index_page_size")
	npage_size := Getstringtoint(page_size) + page

	var article Article
	articles := article.GetArticles(page, npage_size)
...
//测试方法，构建123条记录！
func (article *Article) GetArticles(page, page_size int64) []*Article {
	var articles []*Article
	articles = make([]*Article, 123)
	var i int64
	for i = 0; i < 123; i++ {
	...
	sas := articles[page:page_size]
	return sas
}

func (c *MainController) GetAjaxArticles() {
	beego.Info("测试进入获取文章方法！")
	spage := c.GetString("Page")
	spage_size := beego.AppConfig.String("index_page_size")
	npage := Getstringtoint(spage)
	npage_size := Getstringtoint(spage_size)
	page := (npage + 1) * npage_size
	page_size := page + npage_size
	var article Article
	articles := article.GetArticles(page, page_size)
	beego.Info(page)
	beego.Info(page_size)
	c.Data["Articles"] = articles
	c.TplNames = "ajax.tpl"
}
