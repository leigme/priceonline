...
func Getstringtoint(stringtoint string) int64 {
	i := beego.AppConfig.String(stringtoint)
...
func (c *MainController) Get() {
	...
	ina := Getstringtoint("indexpagination")
	var article Article
	articles := article.GetArticles(0, ina)
...
//测试方法，构建123条记录！
func (article *Article) GetArticles(zi, ipi int64) []*Article {
...
func PagingAlgorithm(amount, pag_size, pages, page, inta int64) {
	pages = amount / pag_size
	page = 1
	inta = 0
}
...
func (c *MainController) GetAjaxArticles() {
	...
	ipi := Getstringtoint("indexpagination")
	var article Article
	articles := article.GetArticles(0, ipi)
	c.Data["Articles"] = articles
	cid := c.GetString("Id")
	pag := c.GetString("Pag")
	nid := Getstringtoint(cid) + 1
	beego.Info(cid)
	beego.Info("=======")
	beego.Info(pag)
	c.Data["Id"] = nid
